...
function aconnect_create_user_password($inputString) {
  ...
  if($pwuser = $DB->get_record('acusers',array('acuser'=> $inputString))){
    $seed = $cryptor->decryptString($pwuser->password, $key);
  }
  else{ //generate password
    $eseed=explode("@",$USER->email);
    $seed=$eseed[0].time();
    $crypted = $cryptor->encryptString($seed, $key);
...
function rewrite_user_password($acname, $newpass, $bool=0){
  ...
  $cryptor = new Cryptor('aes-256-ctr', 'sha256', Cryptor::FORMAT_B64);
    ...
    $record_exists->password = $cryptor->encryptString($newpass, $key);
		...
		$obj = new stdClass();
		$obj->acuser= $acname;
    $obj->password = $cryptor->encryptString($newpass, $key);
...
function check_if_userset_pw($acname){
	...
	if($record_exists = $DB->get_record('acusers', array('acuser'=>$acname))){
      ...
      $cryptor = new Cryptor('aes-256-ctr', 'sha256', Cryptor::FORMAT_B64);
    	$password= $cryptor::Decrypt($record_exists->password, $key) ;
...
/**
 * Updates the timelastvisited-field in the adobeconnect-table to current timestamp for a ac-room with given ID.
 * @param int $acroom_id
 * @return void
 *
 */
function aconnect_update_time_last_visited($acroom_id) {
    global $DB;
    $toupdate = new stdClass();
    $toupdate->id = $acroom_id;
    $toupdate->timelastvisited = time();
